...
public class AggregateIterableMeanInteger extends AbstractAggregateIterableFunction {
    @Override
    public Pair<Object, DataTuple> result(Pair<Object, ArrayList<DataTuple>> accum) {
        return Pair.of(accum.getFirst(), new AggregateMeanFunction<>(Integer.class).result(accum.getSecond()));
    }
}
