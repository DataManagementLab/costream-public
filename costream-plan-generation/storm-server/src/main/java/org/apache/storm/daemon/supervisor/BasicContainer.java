...
public class BasicContainer extends Container {
    ...
    public boolean isMemoryLimitViolated(LocalAssignment withUpdatedLimits) throws IOException {
        ...
        if (resourceIsolationManager.isResourceManaged()) {
            ...
            if (usageMb > hardLimitMb) {
                ...
                //return true;
                return false; // ToDo: Disabling memory checking here as it was not yet updated to Cgroup- containers
                // ToDo: And thus it only relies on RAS / Assignments
            }
    ...
    public void launch() throws IOException {
        ...
        int memOnHeap = getMemOnHeap(resources);
        ...
        if (resourceIsolationManager.isResourceManaged()) {
            ...
            Double totalMemory = resources.get_resources().get("ram");
            Double totalMemSwap = resources.get_resources().get("ramswap");
            Double cpuLimit = resources.get_resources().get("cpu");
            Double networkBandwidth = resources.get_resources().get("bandwidth");
            Double networkLatency = resources.get_resources().get("latency");

            LOG.info("Read resources from assignment: mem: {}, memsw: {}, cpu: {}, network: {}, latency: {}",
                    totalMemory, totalMemSwap, cpuLimit, networkBandwidth, networkLatency);

            Integer majorClassId = null; // setting the classIds to null as we do not need cgroup network modelling here
            Integer minorClassId = null;

            if (totalMemory != -1) {
                memoryLimitMb = totalMemory.longValue();
            }
            ...
            resourceIsolationManager.reserveResourcesForWorker(workerId,
                    ...
                    totalMemSwap.intValue(),
                    cpuLimit.intValue(),
                    cpuShares, majorClassId, minorClassId, numaId);

            String networkInterface = getLocalNetworkInterfaceName();

            // After cgroup values have been set, apply the tc filter to the corresponding ID.
            String logPrefix = "network_filter " + workerId;
            String workerDir = ConfigUtils.workerRoot(conf, workerId);
            ProcessCallback processExitCallback = new ProcessCallback(logPrefix);

            // The following code is designed for a cgroup-based class filter
            // However, this is not necessary if we assume a single worker JVM per host.
            // Then it can be simplified by a common netem call that sets network and bandwidth in parallel.

            /*
            String hexClassId = "0x" + toHex(majorClassId) + toHex(minorClassId);
            LOG.info("Found a net_cls identifier {}:{} or {}, apply bandwidth filter on worker",
                    majorClassId, minorClassId, hexClassId);

            String changeCommand = "sudo tc class change dev " + networkInterface + " parent 10: classid "
                    + majorClassId + ":" + minorClassId + " htb rate " + networkBandwidth.intValue() + "mbit";
            LOG.info("Executing command: {}", changeCommand);
            ClientSupervisorUtils.launchProcess(Arrays.asList(changeCommand.split(" ")),
                    topEnvironment,
                    logPrefix,
                    processExitCallback,
                    new File(workerDir));

            String filterCommand = "sudo tc filter add dev " + networkInterface + " parent 10: "
                    + "protocol ip prio 10 handle 1: cgroup";
            LOG.info("Executing command: {}", filterCommand);
            ClientSupervisorUtils.launchProcess(Arrays.asList(filterCommand.split(" ")),
                    topEnvironment,
                    logPrefix,
                    processExitCallback,
                    new File(workerDir));
            ...
            // tc qdisc add dev ens3 root netem delay 100ms rate 10mbit
            String networkCommand = "sudo tc qdisc change dev " + networkInterface
                    + " root netem delay "
                    + networkLatency.intValue()
                    + "ms "
                    + "rate "
                    + networkBandwidth.intValue()
                    + "mbit";

            LOG.info("Executing command: {}", networkCommand);
            ClientSupervisorUtils.launchProcess(Arrays.asList(networkCommand.split(" ")),
                    topEnvironment,
                    logPrefix,
                    processExitCallback,
                    new File(workerDir));
    ...
    private String getLocalNetworkInterfaceName() {
        // Identify relevant network interface at first
        ...
        while (interfaces.hasMoreElements()) {
            ...
            while (addresses.hasMoreElements()) {
                ...
                if (address.isSiteLocalAddress() && address.getHostAddress().startsWith("192.168.")) {
                    LOG.info("Found local network interface: "
                            + ni.getName() + " with address: "
    ...
    private class ProcessCallback implements ExitCodeCallback {
        private final String logPrefix;

        ProcessCallback(String logPrefix) {
            this.logPrefix = logPrefix;
        }
