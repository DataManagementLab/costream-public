...
public class Stream<T> {
     ...
     */
    public Stream<T> filter(Predicate<? super T> predicate, HashMap<String, Object> description) {
        return new Stream<>(streamBuilder, addProcessorNode(new FilterProcessor<>(predicate, description), VALUE, true));
    }

    public Stream<T> filter(Predicate<? super T> predicate) {
        return new Stream<>(streamBuilder, addProcessorNode(new FilterProcessor<>(predicate, new HashMap<>()), VALUE, true));
    }
     ...
     * @return the new stream
     */
    public <R> Stream<R> map(Function<? super T, ? extends R> function, HashMap<String, Object> description) {
        return new Stream<>(streamBuilder, addProcessorNode(new MapProcessor<>(function, description), VALUE));
    }


    public <R> Stream<R> map(Function<? super T, ? extends R> function) {
        return new Stream<>(streamBuilder, addProcessorNode(new MapProcessor<>(function, new HashMap<>()), VALUE));
    }
    ...
    public <K, V> PairStream<K, V> mapToPair(PairFunction<? super T, ? extends K, ? extends V> function,
        ...
        return new PairStream<>(streamBuilder, addProcessorNode(new MapProcessor<>(function, description), KEY_VALUE));
    }
     ...
     * @return the new stream
     */
    public <A, R> Stream<R> aggregate(CombinerAggregator<? super T, A, ? extends R> aggregator, HashMap<String, Object> description) {
        return combine(aggregator, description);
    }

    public <A, R> Stream<R> aggregate(CombinerAggregator<? super T, A, ? extends R> aggregator) {
        return combine(aggregator, null);
    }
    ...
    public <R> Stream<R> aggregate(R initialValue,
                                   ...
                                   HashMap<String, Object> description) {
        return combine(CombinerAggregator.of(initialValue, accumulator, combiner), description);
    }
    ...
    public Stream<Long> count() {
        return aggregate(new Count<>(), new HashMap<>());
    }
    ...
    public void to(IRichBolt bolt) {
        to(bolt, 1, new HashMap<>());
    }

    public void to(IRichBolt bolt, HashMap<String, Object> description) {
        to(bolt, 1, description);
    }
    /**
     * Sends the elements of this stream to a bolt. This could be used to plug in existing bolts as sinks in the stream, for e.g. a {@code
     * RedisStoreBolt}.
    ...
    public void to(IRichBolt bolt, int parallelism, HashMap<String, Object> description) {
        addSinkNode(new SinkNode(bolt, description), parallelism);
    }
    ...
    public void to(IBasicBolt bolt, int parallelism) {
        addSinkNode(new SinkNode(bolt, new HashMap<>()), parallelism);
    }
    ...
    private <A> Stream<A> combinePartition(CombinerAggregator<? super T, A, ?> aggregator, HashMap<String, Object> description) {
        return new Stream<>(streamBuilder,
                addProcessorNode(new AggregateProcessor<>(aggregator, true, description), VALUE, true));
    }
    ...
    private <A, R> Stream<R> aggregatePartition(CombinerAggregator<? super T, A, ? extends R> aggregator,
                                                HashMap<String, Object> description) {
        return new Stream<>(streamBuilder, addProcessorNode(new AggregateProcessor<>(aggregator, description), VALUE));
    }
    ...
    private <A, R> Stream<R> combine(CombinerAggregator<? super T, A, ? extends R> aggregator, HashMap<String, Object> description) {
        if (shouldPartition()) {
            if (node instanceof ProcessorNode) {
                if (node.isWindowed()) {
                    return combinePartition(aggregator, description).global().merge(aggregator);
                }
                ...
                if (!nonWindowed.isPresent()) {
                    parents.forEach(p -> {
                        Node localAggregateNode = makeProcessorNode(
                                new AggregateProcessor<>(aggregator, true, description), VALUE, true);
                        streamBuilder.insert(p, localAggregateNode);
                    });
                    return ((Stream<A>) global()).merge(aggregator);
                }
            }
            return global().aggregatePartition(aggregator, description);
        } else {
            return aggregatePartition(aggregator, description);
        }
    }
