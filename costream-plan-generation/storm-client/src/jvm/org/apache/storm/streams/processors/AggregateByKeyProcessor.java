...
public class AggregateByKeyProcessor<K, V, A, R> extends BaseProcessor<Pair<K, V>> implements BatchProcessor {
    ...
    public AggregateByKeyProcessor(CombinerAggregator<V, A, R> aggregator, HashMap<String, Object> description) {
        this.aggregator = aggregator;
        this.emitAggregate = false;
    ...
    public void execute(Pair<K, V> input) {
        this.monitor.reportInput(input);
    ...
    @Override
    public void finish() {
        this.monitor.reportOutput(state);
