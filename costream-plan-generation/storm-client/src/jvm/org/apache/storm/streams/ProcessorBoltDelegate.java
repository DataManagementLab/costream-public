...
class ProcessorBoltDelegate implements Serializable {
    ...
    void cleanup() {
        ...
        while (it.hasNext()) {
            ...
            try {
                BaseProcessor processor = (BaseProcessor) ((ProcessorNode) node).getProcessor();
                StreamMonitor monitor = processor.getMonitor();
                if (monitor != null) {
                    monitor.cleanup();
                }
    ...
    private boolean shouldPunctuate(ProcessorNode processorNode, String sourceStreamId) {
        if (!processorNode.getWindowedParentStreams().isEmpty()) {
            ...
            // -- THIS was commented out by RoHei, because otherwise, no three way join is possible!
