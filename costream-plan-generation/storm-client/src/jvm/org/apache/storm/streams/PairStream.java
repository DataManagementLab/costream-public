...
public class PairStream<K, V> extends Stream<Pair<K, V>> {
    ...
    public <R> PairStream<K, R> aggregateByKey(R initialValue,
        ...
        return combineByKey(CombinerAggregator.of(initialValue, accumulator, combiner), new HashMap<>());
    }
    ...
    public <A, R> PairStream<K, R> aggregateByKey(CombinerAggregator<? super V, A, ? extends R> aggregator,
        ...
        return combineByKey(aggregator, description);
    }
    ...
    public PairStream<K, Long> countByKey() {
        return aggregateByKey(new Count<>(), new HashMap<>());
    }
    ...
    public PairStream<K, Iterable<V>> groupByKey(HashMap<String, Object> description) {
        return partitionByKey().aggregatePartition(new MergeValues<>(), description);
    }
     ...
     * {@inheritDoc}
     */
    public PairStream<K, V> filter(Predicate<? super Pair<K, V>> predicate, HashMap<String, Object> description) {
        return toPairStream(super.filter(predicate, description));
    }
     ...
     * @return the new stream
     */
    public <V1> PairStream<K, Pair<V, V1>> join(PairStream<K, V1> otherStream, HashMap<String, Object> description) {
        return join(otherStream, new PairValueJoiner<>(), description);
    }

    public <V1> PairStream<K, Pair<V, V1>> join(PairStream<K, V1> otherStream) {
        return join(otherStream, new PairValueJoiner<>(), new HashMap<>());
    }
    ...
    private <R, V1> PairStream<K, R> joinPartition(PairStream<K, V1> otherStream,
        ...
        Node joinNode = addProcessorNode(
                new JoinProcessor<>(leftStream, rightStream, valueJoiner, leftType, rightType, description),
                KEY_VALUE,
                true);
    ...
    private <A, R> PairStream<K, R> aggregatePartition(
            CombinerAggregator<? super V, A, ? extends R> aggregator, HashMap<String, Object> description) {
        return new PairStream<>(streamBuilder,
                addProcessorNode(new AggregateByKeyProcessor<>(aggregator, description), KEY_VALUE, true));
    }
    ...
    private <A, R> PairStream<K, R> combineByKey(CombinerAggregator<? super V, A, ? extends R> aggregator,
                                                 HashMap<String, Object> description) {
        // todo - insert description into corresponding nodes
