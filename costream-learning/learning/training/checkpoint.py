...
def save_checkpoint(epochs_wo_improvement, epoch, model, optimizer, target_path, model_name, metrics,
    ...
    if metrics is not None:
        for metrics_by_type in metrics.values():
...
def load_checkpoint(model, target_path, model_name, filetype='.pt', optimizer=None, metrics=None, device=None):
    """
    ...
    if target_path is not None and model_name is not None:
        try:
            checkpoint = torch.load(os.path.join(target_path, f'{model_name}{filetype}'),  map_location=torch.device(device))
