 ...
 */
public class SmartPlug implements Serializable {
    ...
    private final int loadOscillation;
    private final double probabilityOn;
    private final int[] onLenghts;
    ...
    public void tryToSetOn(long currentTimestamp) {
        if (on) {
            if (currentTimestamp - since > howLong) {
                ...
                howLong = onLenghts[RanGen.randInt(0, onLenghts.length - 1)];
                canBeChanged = false;
            }
        } else {
            if (currentTimestamp - since > howLong) {
    ...
    public double getLoad() {
        if (on) {
            ...
            int load = RanGen.randInt(min, max);
